(function() {
            ...
            function syncHeaderState() {
                ...
                header.classList.toggle('header--menu-open', isMenuOpen);
            ...
            const projetos = [ 
                ...
                { id: 1, titulo: 'Becker', descricao: 'Site premium para salão de beleza com serviços, loja, galeria, agendamento e contato direto pelo WhatsApp.', categoria: 'sites', icone: '✨', techs: ['Beleza premium', 'Agendamento', 'Loja online'], tag: 'Site', imagem: 'portfolio-becker.webp', url: 'https://salaobecker.com.br', destaque: true }, 

                { id: 2, titulo: 'Rayanni | Galeria Virtual', descricao: 'Galeria virtual autoral para apresentar artes, ilustrações, escrita e nécessaires com experiência imersiva e atendimento pelo WhatsApp.', categoria: 'sites', icone: '🎨', techs: ['Galeria virtual', 'Catálogo autoral', 'WhatsApp'], tag: 'Site', imagem: 'portfolio-rayanni.webp', url: 'https://rayanni.com.br', destaque: true }, 

                { id: 3, titulo: 'Fort.Art Papelaria e Presentes', descricao: 'Site para papelaria com apresentação da marca, produtos criativos e chamada para visita e contato.', categoria: 'sites', icone: '✏️', techs: ['Site institucional', 'Papelaria', 'Responsivo'], tag: 'Site', imagem: 'portfolio-fortart.webp', url: 'https://fortart.com.br', destaque: true }, 

                { id: 4, titulo: 'Mayle Store', descricao: 'Loja online de moda feminina com vitrine de produtos, identidade visual e experiência responsiva.', categoria: 'sites', icone: '🛍️', techs: ['Loja online', 'Catálogo', 'Responsivo'], tag: 'Site', imagem: 'portfolio-mayle-store.webp', url: 'https://contatokaleomiranda-netizen.github.io/mayle-store/' }, 

                { id: 5, titulo: 'Pousada Estrela da Manhã', descricao: 'Site institucional para apresentar a pousada, destacar seus espaços e direcionar reservas pelo WhatsApp.', categoria: 'sites', icone: '🏡', techs: ['Site institucional', 'Responsivo', 'WhatsApp'], tag: 'Site', imagem: 'portfolio-pousada.jpg', url: 'https://contatokaleomiranda-netizen.github.io/Pousada-Estrela-da-Manh-/' }, 

                { id: 6, titulo: 'Estoque Fort.Art', descricao: 'Sistema interno para controle de produtos, categorias, indicadores e alertas de baixo estoque.', categoria: 'sistemas', icone: '📦', techs: ['Sistema de estoque', 'Painel interno', 'Gestão de produtos'], tag: 'Sistema', imagem: 'portfolio-estoque-fortart.jpg', url: 'https://contatokaleomiranda-netizen.github.io/Estoque-da-papelaria/' }, 

                { id: 7, titulo: 'Dashboard de Vendas', descricao: 'Painel de indicadores comerciais com visão de faturamento, pedidos, ticket médio, categorias e cidades.', categoria: 'dashboards', icone: '📊', techs: ['Power BI', 'Indicadores', 'Vendas'], tag: 'Dashboard', imagem: 'portfolio-dashboard-vendas.svg', url: '#contato', actionLabel: 'Orçar no formulário' }, 

                { id: 8, titulo: 'Painel Analítico 360', descricao: 'Dashboard multipágina para acompanhar vendas, clientes, produtos, canais e eficiência operacional.', categoria: 'dashboards', icone: '📈', techs: ['Power BI', 'BI 360', 'Análise gerencial'], tag: 'Dashboard', imagem: 'portfolio-dashboard-bi.svg', url: '#contato', actionLabel: 'Orçar no formulário' } 

            ]; 

            const portfolioFeaturedGrid = document.getElementById('portfolioFeaturedGrid'); 
            const portfolioGrid = document.getElementById('portfolioGrid'); 
            const portfolioAllWrapper = document.getElementById('portfolioAllWrapper'); 
            const togglePortfolioBtn = document.getElementById('togglePortfolioBtn'); 
            const projetosDestaque = projetos.filter(function(projeto) { return projeto.destaque; }).slice(0, 3); 

            function markupProjeto(projeto) { 
                return '<div class="project-card reveal" data-3d>' + 
                    '<div class="project-card__img"><img class="project-card__shot" src="' + imageBasePath + projeto.imagem + '" alt="Prévia do projeto ' + projeto.titulo + '" loading="lazy" decoding="async" width="960" height="667"><span class="project-card__tag">' + projeto.tag + '</span></div>' + 
                    '<div class="project-card__info"><h3>' + projeto.titulo + '</h3><p>' + projeto.descricao + '</p><div class="project-card__techs">' + projeto.techs.map(function(tech) { return '<span>' + tech + '</span>'; }).join('') + '</div><div class="project-card__actions"><a class="project-card__link" href="' + projeto.url + '"' + (projeto.url.charAt(0) === '#' ? '' : ' target="_blank" rel="noopener noreferrer"') + '>' + (projeto.actionLabel || 'Ver projeto') + ' <i class="fas ' + (projeto.url.charAt(0) === '#' ? 'fa-arrow-down' : 'fa-arrow-up-right-from-square') + '" aria-hidden="true"></i></a><a class="project-card__link project-card__link--ghost" href="' + whatsappUrl(getSimilarProjectMessage(projeto)) + '" target="_blank" rel="noopener noreferrer">Quero parecido</a></div></div></div>'; 
            }

            function ativarTiltProjeto(card) {
                if (card.getAttribute('data-tilt-ready') === 'true') {
                    return;
                }
            ...
            function prepararCardsProjeto(container) {
                if (!container) {
                    return;
                }

                container.querySelectorAll('.project-card.reveal').forEach(function(card) {
                ...
                container.querySelectorAll('.project-card[data-3d]').forEach(function(card) {
                    ativarTiltProjeto(card);
                });
            }

            function renderizarProjetosDestaque() {
                if (!portfolioFeaturedGrid) {
                    return;
                }

                portfolioFeaturedGrid.innerHTML = projetosDestaque.map(function(projeto) {
                    return markupProjeto(projeto);
                }).join('');

                prepararCardsProjeto(portfolioFeaturedGrid);
            }
            ...
            function renderizarPortfolio(filtro) { 
                ...
                setTimeout(function() { 
                    ...
                    const filtrados = cat === 'todos' ? projetos : projetos.filter(function(projeto) { 

                        return projeto.categoria === cat; }); 

                    portfolioGrid.innerHTML = filtrados.map(function(projeto) { 
                    ...
                    prepararCardsProjeto(portfolioGrid); 

                }, 200); 

            } 

            function atualizarBotaoPortfolio(expandido) {
                if (!togglePortfolioBtn) {
                    return;
                }

                togglePortfolioBtn.setAttribute('aria-expanded', expandido ? 'true' : 'false');
                togglePortfolioBtn.innerHTML = expandido ?
                    'Ocultar todos os projetos <i class="fas fa-arrow-up" aria-hidden="true"></i>' :
                    'Ver todos os projetos <i class="fas fa-arrow-down" aria-hidden="true"></i>';
            }
            ...
            if (togglePortfolioBtn && portfolioAllWrapper) {
                togglePortfolioBtn.addEventListener('click', function() {
                    const expandido = portfolioAllWrapper.hidden;
                    portfolioAllWrapper.hidden = !expandido;
                    atualizarBotaoPortfolio(expandido);

                    if (expandido) {
                        setTimeout(function() {
                            portfolioAllWrapper.scrollIntoView({ behavior: 'smooth', block: 'start' });
                        }, 120);
                    }
                });
            }

            renderizarProjetosDestaque();
            renderizarPortfolio('todos'); 
            atualizarBotaoPortfolio(false);
